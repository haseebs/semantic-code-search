...
class Evaluator:
    ...
    def add_sample(self, index, rank_dict, samples):
        ...
        if index < NUMBER_OF_OUTPUT_SAMPLES:
            original_code_rank = original_code_rank if original_code_rank != bad_rank else 'very bad'
            matching_tokens = self.tokenizer.token_intersection(doc_sample['docstring_tokens'], doc_sample['code_tokens'])

            result = f"#{index}. Function is ranked {original_code_rank}\n"
            result += f"- Docstring tokens: '{' '.join(doc_sample['docstring_tokens'])}'\n"
            ...
            for rank in range(1, 4):
                if rank in rank_dict:
                    ...
                    matching_tokens = self.tokenizer.token_intersection(doc_sample['docstring_tokens'],
                                                                        code_sample['code_tokens'])

                    result += f"###{rank}. Rank: {rank_dict[rank]}. Function\n"
                    result += f"- matching tokens ({len(matching_tokens)}): '{' '.join(matching_tokens)}'\n"
                    result += f"- all code tokens: '{' '.join(code_sample['code_tokens'])}'\n"
        ...
        if index == len(samples)-1:
            ...
            self.writer.add_text("Mean rank", f"{str(self.mean_rank())} of {len(self.original_ranks)} samples")
    ...
    def mrr(self):
        return sum([1/rank for rank in self.original_ranks]) / len(self.original_ranks)

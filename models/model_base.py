...
class ModelBase(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        query_embs = self.query_encoder(seq_tokens=batch['encoded_query'],
    ...
    def validation_epoch_end(self, out):
        ...
        avg_mrr = torch.stack([x['mrr'] for x in out]).mean() #TODO mean of means != total mean
    ...
    def init_encoders(self):
        ...
        for sample in self.train_dataset.original_data:
            ...
            self.query_encoder.update_tokens_from_sample([t.lower() for t in sample['docstring_tokens']])
    ...
    def get_eval_metrics(self, code_embs: torch.tensor, query_embs: torch.tensor):
        ...
        per_sample_loss = torch.max(torch.tensor(0.).cuda(),
                                    self.hypers['margin']
                                    - similarity_scores.diagonal()
                                    + torch.max(F.relu(similarity_scores + neg_matrix.cuda()),
                                                dim=-1)[0])
    ...
    def train_dataloader(self):
        if len(self.train_dataset.encoded_data) == 0: #TODO Why is this being called more than once

        ...
        for path in paths:
            ...
            for data_file in data_files:
                if data_file.endswith('.jsonl.gz'):
                    self.original_data.extend(self.read_jsonl(path=os.path.join(path, data_file)))

    def encode_data(self, query_encoder: EncoderBase, code_encoder: EncoderBase) -> None: #TODO may need to move to encoder class to handle encoders that come with their own tokenizers
        for sample in self.original_data:
            enc_query, enc_query_mask = convert_and_pad_token_sequence(query_encoder.vocabulary,
                                                                       sample['docstring_tokens'],
                                                                       self.hypers['query_max_num_tokens'])
            enc_code, enc_code_mask = convert_and_pad_token_sequence(code_encoder.vocabulary,
                                                                     sample['code_tokens'],
            ...
            self.encoded_data.append({'encoded_query': enc_query,
...
if __name__ == '__main__':
    dataset = CSNDataset()
    encoder_code = EncoderBase({'use_bpe': True,
                                'vocab_size': 10000,
                                'vocab_pct_bpe': 0.5,
                                'vocab_count_threshold': 10})
    encoder_query = EncoderBase({'use_bpe': True,
                                 'vocab_size': 10000,
                                 'vocab_pct_bpe': 0.5,
                                 'vocab_count_threshold': 10})
    ...
    for sample in dataset:
        ...
        encoder_query.update_tokens_from_sample([t.lower() for t in sample['docstring_tokens']])
